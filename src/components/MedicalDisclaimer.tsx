...
export default function MedicalDisclaimer({ compact = false }: MedicalDisclaimerProps) {
  if (compact) {
    return (
      <div className="flex items-start gap-2 p-2.5 rounded-xl bg-muted/60 border border-border/50">
        <Info className="w-3.5 h-3.5 text-muted-foreground shrink-0 mt-0.5" />
        <p className="text-xs text-muted-foreground leading-relaxed">
          <span className="font-semibold">Disclaimer:</span> For educational purposes only.
  ...
  return (
    <div className="p-4 rounded-2xl bg-muted/60 border border-border/50">
      <div className="flex items-center gap-2 mb-2">
        ...
        <h4 className="font-display font-bold text-xs text-muted-foreground uppercase tracking-wide">Medical Disclaimer</h4>
      </div>
      <p className="text-xs text-muted-foreground leading-relaxed">
